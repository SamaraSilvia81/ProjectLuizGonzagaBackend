...
const preprocessText = (text, dictionary) => {
    ...
    for (let i = 0; i < inputLength; i++) {
        ...
        // Verificar se o caractere está presente no dicionário
        if (numericValue !== undefined) {
            numericValues.push(numericValue);
        } else {
            // Caractere desconhecido - você pode definir um valor especial ou realizar outra ação adequada
...
const makePrediction = async (inputData) => {
    try {
      ...
      const inputTensor = tensor2d(inputNumeric, [128, inputNumeric.length]);
  
      // Faça a previsão com o tensor de entrada
      const outputTensor = AIModel.predict(inputTensor);
  
      // Obtenha os valores da previsão como um array JavaScript
      const outputData = await outputTensor.array();
  
      // Lembre-se de liberar os tensores para liberar memória
      inputTensor.dispose();
...
const decodeOutput = (outputData, reverseDictionary) => {
  ...
  for (let i = 0; i < outputLength; i++) {
    ...
    // Verificar se o valor numérico está presente no dicionário invertido
    if (char !== undefined) {
      decodedOutput.push(char);
    } else {
      // Valor numérico desconhecido - você pode definir um caractere especial ou realizar outra ação adequada
...
app.post('/generate', async (req, res) => {
    const {
        text
    } = req.body; // Obter a quantidade de palavras desejada
    const count = 100
    ...
    try {
        ...
        for (let i = 0; i < count; i++) {
            ...
            AIResponse[argmaxIndex] = -Infinity; // Definir o valor como -Infinity para evitar que seja selecionado novamente
        }
